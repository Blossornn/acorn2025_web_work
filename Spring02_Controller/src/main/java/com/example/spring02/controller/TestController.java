...
// @Controller 어노테이션으로 bean 이 된 객체는 클라이언트의 요청을 처리 할 수 있다.
@Controller
public class TestController {
	...
	/*
	 *  @RequestParam 어노테이션을 이용해서 추출 된 요청 파라미터를 전달 받을 수 있다.
	 *  매개 변수 명을 input 요소의 name 속성의 값과 일치 시키면 @RequestParam 은 생략 가능
	...
	@PostMapping("/send")
	@ResponseBody
	public String send(HttpServletRequest request) { // HttpServletRequest 객체가 필요하다면 매개변수에 선언하면 객체가 전달된다
		// 요청 파라미터 추출
