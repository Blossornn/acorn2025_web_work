...
@Controller // 이 어노테이션이 있어서 HomeController 객체는 자동 생성되고 bean 으로 관리
public class HomeController {
	
	
	// @Controller 어노테이션으로 bean이 되는 객체는 클라이언트의 요청을 처리 할 수 있다.
	@GetMapping("/")
	public String home(Model model) { // 컨트롤러 메소드의 매개변수에 Model 을 선언하면 객체가 자동으로 전달 된다
		// 응답에 필요한 데이터는 Model 객체에 담는다.
		// Model 객체에 담으면 자동으로 HttpServletRequest 객체에 담긴다.
		List<String> notice = List.of("공지사항 입니다!", "어쩌구...", "저쩌구...");
		// "notice"라는 키 값으로 List<String> type을 담는다.
		model.addAttribute("notice", notice);
		/*
		 *  여기서 "home" 이라는 문자열을 리턴하면
		 *  접두어 "/WEB-INF/views/" 가 붙고
