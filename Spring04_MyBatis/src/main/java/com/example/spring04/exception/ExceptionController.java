...
// 예외 처리를 할 컨트롤러에는 @ControllerAdvice 라는 어노테이션을 붙여준다.
@ControllerAdvice
public class ExceptionController {
	/*
	 *  spring 프레임워크가 동작하는 과정 중에 DataAccessException type의 예외가 발생하면 
	 *  이 메소드가 자동으로 호출 된다.
	...
	public String dataAccess(DataAccessException dae, Model model) {
		...
		// /WEB-INF/views/error/data-access.jsp 페이지에서 에러 정보를 응답
 		return "error/data-access";
	}
	
	@ExceptionHandler(MemberException.class)
	public String memberExeception(MemberException me, Model model) {
		model.addAttribute("title", "Member 관련 작업 중 에러 발생!");
