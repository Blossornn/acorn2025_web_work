 ...
 *
 *  2. 필요한 객체가 있다면 Dependency Injection(DI) 방식으로 받아서 사용
 *
 *  3. Interface type 을 (핵심 의존 객체의 type) 을 적극 활용한다.
 *
 *  위의 세 가지 원칙을 지키면 객체들 간의 의존 관계가 느슨해져서 유지 보수가 편하게 된다
 */

@SpringBootApplication // 이 어노테이션떄문에 Spring01HelloApplication 객체도 생성이 되고 관리가 된다.
public class Spring01HelloApplication {

	// 필요한 핵심 의존 객체를 필드로 선언하고 DI 되도록 설정(Interface type으로 선언)
	...
	public static void main(String[] args) {
		...
		// "원숭이" 의 집을 청소 해야한다면 어떻게 해야 할까?
		//HomeServiceImpl service1 = new HomeServiceImpl();
		//service1.clean("원숭이");

		//Spring이 관리하는 객체중에 HomeService type 객체를 얻어낸다
